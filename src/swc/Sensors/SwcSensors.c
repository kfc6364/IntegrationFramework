...
#define SWCSENSORS_SW_MAJOR_VERSION_C                   0
#define SWCSENSORS_SW_MINOR_VERSION_C                   8
...
void SwcSensors_Init(const SwcRte_GlobalConfigType * ConfigPtr)
{
    ...
    for(RteAppInstance = 0U; RteAppInstance < SWCRTE_MAX_APP_CTRL_INSTANCES; RteAppInstance++)
    {
...
{
    volatile SwcRte_AppChannelIdType SensorChannelId;
    volatile SwcRte_AppInstanceIdType RteAppInstance;
    volatile SwcRte_AppInputSensorType SensorType;
    SwcRte_AppInputValueType InputValue;
#if (SWCRTE_LIGTHING_APP_ENABLED == SWCRTE_STD_ON)        
    SwcRte_AppLightingSensorValueType  AppLightingWriteValue;
#endif
#if (SWCRTE_MOTORCTRL_APP_ENABLED == SWCRTE_STD_ON)    
   SwcRte_AppMotorCtrlSensorValueType  AppMotorCtrlWriteValue;
#endif
    ...
    for (RteAppInstance = 0; RteAppInstance < SWCRTE_MAX_APP_CTRL_INSTANCES; RteAppInstance++)
    {
    #if SWCRTE_MAX_DISENS_INSTANCES > 0        
        AppLightingWriteValue.LightInputSensorData.bSensorValueUpdated = SWCRTE_FALSE;
    #endif
        ...
        {                   
            ...
            {                
                ...
                {                                
                    AppLightingWriteValue.LightInputSensorData.u32LightingSensorValue = 0;
                    ...
                    {    
                        ...
                        {    
                            ...
                            SwcSensors_DioSensorHighValueCounter[RteAppInstance] = 0U;   
                            AppLightingWriteValue.LightInputSensorData.bSensorValueUpdated = SWCRTE_TRUE;                            
                        }                        
                    }
                    else
                    {    
                    ...
                    {                   
                        AppLightingWriteValue.LightInputSensorData.u32LightingSensorValue = SensorValue;                          
                    }
                    #endif    
                ...
                {    
                    if(SWCRTE_MEAS_OK == InputValue.Status)
                    {
                        ...
                        if(SWCRTE_LIGHTING == SwcSensors_ConfigPtr[RteAppInstance].eInstanceType)
                        { 
                            AppLightingWriteValue.LightInputSensorData.u32LightingSensorValue = InputValue.Value;
                ...
                case SWCRTE_ANALOG_FDBK_SENS:            
                {
                    ...
                    if(SWCRTE_LIGHTING == SwcSensors_ConfigPtr[RteAppInstance].eInstanceType)
                    {
                        AppLightingWriteValue.aLightChannelSensorData[SensorChannelId - 1U].u32LightingSensorValue = InputValue.Value;
                        
                        if(InputValue.Status == SWCRTE_MEAS_OK)
                        {
                             AppLightingWriteValue.aLightChannelSensorData[SensorChannelId - 1U].bSensorValueUpdated = SWCRTE_TRUE; 
                        }
                        else
                        {
                            AppLightingWriteValue.aLightChannelSensorData[SensorChannelId - 1U].bSensorValueUpdated = SWCRTE_FALSE; 
                        }    
                    }
                    #endif
                    #if (SWCRTE_MOTORCTRL_APP_ENABLED == SWCRTE_STD_ON)                    
